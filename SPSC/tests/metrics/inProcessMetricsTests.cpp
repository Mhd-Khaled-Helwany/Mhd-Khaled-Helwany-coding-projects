...
3) Run the tests directly or via CTest:
    ./build/MetricsTests
    # or
    ctest --test-dir build -R MetricsTests -V
*/
...
TEST_CASE(test_register_strategy_returns_handle) {
    ...
    auto handle = recorder.registerStrategy(SpscRing<MetricEvent>(16), 1234);
...
TEST_CASE(test_drop_newest_rejects_when_full) {
    ...
    MetricsRecorder recorder(config, publisher);
    auto handle = recorder.registerStrategy(SpscRing<MetricEvent>(config.queueCapacity), 1);
    REQUIRE_TRUE(handle != nullptr);
...
TEST_CASE(test_drop_oldest_accepts_extra_event) {
    ...
    MetricsRecorder recorder(config, publisher);
    auto handle = recorder.registerStrategy(SpscRing<MetricEvent>(16), 2);
    REQUIRE_TRUE(handle != nullptr);
...
TEST_CASE(test_next_sequence_is_monotonic) {
    ...
    RecorderConfig config = makeConfig(DropPolicy::DropNewest);
    MetricsRecorder recorder(config, publisher);
    auto handle = recorder.registerStrategy(SpscRing<MetricEvent>(16), 3);
    REQUIRE_TRUE(handle != nullptr);
...
TEST_CASE(test_recorder_publishes_events) {
    ...
    RecorderConfig config = makeConfig(DropPolicy::DropNewest);
    MetricsRecorder recorder(config, publisher);
    auto handle = recorder.registerStrategy(SpscRing<MetricEvent>(16), 42);
    REQUIRE_TRUE(handle != nullptr);
