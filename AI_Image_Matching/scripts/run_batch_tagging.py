...
Run: `py scripts/run_batch_tagging.py`
"""
...
from telemetry.budget import assert_budget
from telemetry.call_logger import clear_calls, record_call
from vision.gemini_client import call_gemini_for_tag
from db.session import make_session_factory
from matching.repository import persist_ai_call
import os

def make_db_recorder(session):
    """Record to log + in-memory buffer AND persist into ai_call_log."""
    def _record(rec: dict) -> None:
        record_call(rec)
        persist_ai_call(session, rec)
    return _record
...
def main():
    ...
    with make_session_factory()() as session:
        if use_real:
            ...
            db_record = make_db_recorder(session)

            def real_worker(filename: str):
                assert_budget(session)
            ...
            record_calls = True
            recorder = make_db_recorder(session)

        result = process_batch(batch, worker, max_retries=2, record_calls=record_calls, call_recorder=recorder)
        session.commit()
