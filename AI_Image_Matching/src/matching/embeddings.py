...
def estimate_tokens(text: str) -> int:
    """Standard chars/4 approximation for endpoints that do not report usage."""
...
def embed_text(text: str, *, model: str | None = None, timeout: int = 60) -> dict[str, Any]:
    ...
    if input_tokens == 0 and output_tokens == 0:
        # the embed_content endpoint does not report usage via this SDK;
        # attribute an estimate so every call still carries a token count
