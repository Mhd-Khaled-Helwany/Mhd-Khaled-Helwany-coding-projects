...
collision_types = {"bullet": 1, "tank": 2, "box": 3}     # collision types for the handlers
hit_points = 2


class Bullet(GamePhysicsObject):

    # Constant values for the bullet
    ACCELERATION = 1

    def __init__(self, x, y, orientation, sprite, space, collision_types, shooting_tank):
        super().__init__(x, y, orientation, sprite, space, True)
        self.shape.collision_type = collision_types          # bullet collision type
        self.shooting_tank = shooting_tank                   # attribute that indicates which tank shot the bullet

    def apply_recoil(self):
        """
        A function that gets called to apply a slight recoil to the shooting tank
        """
        recoil_vector = -(pymunk.Vec2d(0, 5).rotated(self.shooting_tank.body.angle))
        self.shooting_tank.body.velocity += recoil_vector

    def update(self):
        """ A function to update the objects coordinates. Gets called at every tick of the game. """
        if self.shooting_tank.buff:
            acceleration_vector = pymunk.Vec2d(0, self.ACCELERATION * 1.4).rotated(self.body.angle)
        else:
            acceleration_vector = pymunk.Vec2d(0, self.ACCELERATION).rotated(self.body.angle)
...
class Tank(GamePhysicsObject):
    ...
    def __init__(self, x, y, orientation, sprite, space, collision_types, player_id, buff, is_ai, hit_points):
        super().__init__(x, y, orientation, sprite, space, True)
        ...
        self.current_time = 50   # Initialize the time of the last shot
        self.shape.collision_type = collision_types["tank"]
        self.is_ai = is_ai
        self.player_id = player_id     # Gives the tank an id number to identify it for example in a scoreboard
        self.orientation = orientation
        self.buff = buff               # applies a buff if it is True
    ...
    def post_update(self):
        ...
        # Else ensure that the tank has its normal max speed
            ...
            if self.buff:
                self.max_speed = Tank.NORMAL_MAX_SPEED * 1.25
            else:
    ...
    def try_grab_flag(self, flag):
        ...
        if not flag.is_on_tank:
            ...
            if (flag_pos - self.body.position).length < 0.5:
                # Grab the flag !
                # Flag grabbed sound
                flag_picked_up = mixer.Sound('flag-pickup.wav')
                flag_picked_up.play()
    ...
    def shoot(self, space, game_object_list):
        """ Call this function to shoot a bullet"""

        if self.current_time >= 50:            # tracking the time to limit the fire rate to one bullet per second
            # Gunshot sound
            expl_sound1 = mixer.Sound('gun-shot.wav')
            expl_sound1.play()

            offset = pymunk.Vec2d(0, 0.1).rotated(self.body.angle)
            shooting_tank = self   # prevents a tank from shooting itself
            bullet = Bullet(self.body.position[0] + offset.x, self.body.position[1] + offset.y,
                            -self.screen_orientation(), images.bullet, self.body.space, 1, shooting_tank)
            bullet.apply_recoil()
            game_object_list.append(bullet)
...
class Box(GamePhysicsObject):
    ...
    def __init__(self, x, y, sprite, movable, space, destructable, collision_types, hit_points):
...
def get_box_with_type(x, y, type, space):
    ...
    if type == 1:  # Creates a non-movable non-destructable rockbox
        return Box(x, y, images.rockbox, False, space, False, collision_types, hit_points)
    if type == 2:  # Creates a movable destructable woodbox
        return Box(x, y, images.woodbox, True, space, True, collision_types, hit_points)
    if type == 3:  # Creates a movable non-destructable metalbox
        return Box(x, y, images.metalbox, True, space, False, collision_types, hit_points)
