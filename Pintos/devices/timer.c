...
/* sleep comparison */
bool sleep_comparison(const struct list_elem *e, const struct list_elem *e_next, void* aux){
	(void) aux; // ignore aux
	struct sleeping_thread *st_1 = list_entry(e, struct sleeping_thread, elem);
	struct sleeping_thread *st_2 = list_entry(e_next, struct sleeping_thread, elem);
	return st_1->wake_up_tick < st_2->wake_up_tick;
}
...
void timer_sleep(int64_t ticks)
{
	...
	enum intr_level old_level = intr_disable();
	list_insert_ordered(&thread_list,&st.elem,sleep_comparison, NULL);
    thread_block();
	intr_set_level(old_level);
	//while (timer_elapsed(start) < ticks){} thread_yield(); busy wait that had to be removed
}
